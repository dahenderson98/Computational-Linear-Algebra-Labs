...
Original file is located at
    https://colab.research.google.com/drive/1TcTYthaLwyQk5UNkxTvJ3y6jMQo5zTls
...
def row_echelon(A,b):  #*****HELP*****
  Ab=augment(A,b)
  C=np.copy(Ab)
  NUM_ROWS,NUM_COLS = np.shape(Ab)
  for j in range (0,NUM_COLS):
    for i in range(j+1, NUM_ROWS):
      C[i,j:]=C[i,j:]-(( C[i,j] / C[j,j] )*C[j,j:])
      #Ab=np.copy(C)
  return C
