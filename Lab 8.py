...
Original file is located at
    https://colab.research.google.com/drive/1kfwbV-5TisKS3yhCknbHj9ahQDdnYTxa
...
def transform(x):
  original = np.array(x)
  new = np.array([0,0,0])
  new[0] = 2 * original[0] + original[1]
  new[1] = original[0] - (3 * original[1])
  new[2] = original[1]
  return new
...
def stretch(image,a,b): 
  stretchVec = np.array([[a,0],[0,b]])
  newImage = np.matmul(stretchVec,image)
  return newImage
...
def shear(image,a,b): 
  shearVec = np.array([[1,a],[b,1]])
  newImage = np.matmul(shearVec,image)
  return newImage
...
def reflect(image,a,b): 
  reflectVec = (1/(np.power(a,2)+np.power(b,2)))*np.array([[np.power(a,2)-np.power(b,2),2*a*b],[2*a*b,np.power(b,2)-np.power(a,2)]])
  newImage = np.matmul(reflectVec,image)
  return newImage
...
def rotate(image,theta): 
  rotateVec = np.array([[np.cos(theta),-np.sin(theta)],[np.sin(theta),np.cos(theta)]])
  newImage = np.matmul(rotateVec,image)
  return newImage
...
#2y=-3x
comp_matrix=stretch(reflect(rotate(stretch(iden,1,2),-np.pi/4),1,-3/2),1/2,1)
...
def area(A): 
  ...
  b=A[1,:]
  ab=a[0]*b[0]+a[1]*b[1]
  alen=np.sqrt(a[0]*a[0]+a[1]*a[1])
  blen=np.sqrt(b[0]*b[0]+b[1]*b[1])
  Area=alen*blen*np.sqrt(np.power(alen*blen,2)-np.power(ab,2))/(alen*blen)
  return Area
...
ellipse_matrix=np.array([[1,1.5],[-2.75,0.5]])
